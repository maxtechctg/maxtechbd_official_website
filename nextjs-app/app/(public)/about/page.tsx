          ...
          <div className="container">
            <div className="row">
              <div className="col-lg-8">
                <div className="subtitle s2 wow fadeInUp mb-3">{data.teamContent?.subtitle || ''}</div>
                <h2 className="wow fadeInUp" data-wow-delay=".2s">{data.teamContent?.title || ''}</h2>
              </div>
            </div>
            <div className="row">
              {data.teamMembers.map((member) => (
                <div key={member.id} className="col-lg-3 col-md-6 text-center mb-sm-20">
                  ...
                  <div className="social-icons s2 mt-2">
                    {member.facebook && (
                      <a href={member.facebook}><i className="fa-brands fa-facebook-f"></i></a>
                    )}
                    {member.twitter && (
                      <a href={member.twitter}><i className="fa-brands fa-twitter"></i></a>
                    )}
                    {member.discord && (
                      <a href={member.discord}><i className="fa-brands fa-discord"></i></a>
                    )}
                    {member.instagram && (
                      <a href={member.instagram}><i className="fa-brands fa-instagram"></i></a>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="row">
              <div className="col-lg-12">
                <div className="padding60 sm-padding40 rounded-30 jarallax text-light">
                  <img src={data.visionSection?.backgroundImage || '/images/background/2.webp'} className="jarallax-img" alt="" />
                  <div className="row">
                    <div className="col-lg-12">
                      <div className="subtitle s2 wow fadeInUp mb-3">{data.visionSection?.subtitle || 'Our Vision'}</div>
                      <h2 className="mb20 wow fadeInUp" data-wow-delay=".2s">{data.visionSection?.heading || 'We aspire to create a digital landscape where technology seamlessly enhances productivity, connectivity, and sustainability, fostering a brighter, more efficient, and inclusive future for all.'}</h2>
                      <a className="btn-main" href={data.visionSection?.buttonUrl || '/contact'}>{data.visionSection?.buttonText || "Let's Work Together"}</a>
                    </div>
                  </div>
                </div>
              </div>
